function canMove(cells) {
  return cells.some((group) => group.some((cell, index) => {
    ...
    return moveToCell.canAccept(cell.value);
  }));
}
...
export default {
  ...
  emptyCells(state) {
    const cells = state.grid.filter((cell) => !cell.value);
    debugger;
    return cells;
  },
  occupiedCells(state) {
    return state.grid.filter((cell) => cell.value);
  },
  grid(state) {
    return state.grid;
  },
  ...
  cellsByRow(state) {
    return state.grid.reduce((cellGrid, cell) => {
      // eslint-disable-next-line no-param-reassign
  ...
  cellsByRowReversed(state, getters) {
    return getters.cellsByRow.map((row) => [...row].reverse());
  },
  cellsByColumn(state) {
    return state.grid.reduce((cellGrid, cell) => {
      // eslint-disable-next-line no-param-reassign
  ...
  cellsByColumnReversed(state, getters) {
    return getters.cellsByColumn.map((column) => [...column].reverse());
  },
  canMoveUp(state, getters) {
    return canMove(getters.cellsByColumn);
  },
  canMoveDown(state, getters) {
    return canMove(getters.cellsByColumnReversed);
  },
  canMoveLeft(state, getters) {
    return canMove(getters.cellsByRow);
  },
  canMoveRight(state, getters) {
    return canMove(getters.cellsByRowReversed);
  },
  gameOver(state) {
