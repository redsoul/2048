export default {
    ...
    cellAvailable: (state, getters) => (cell) => {
        return !getters.cellOccupied(cell);
    },
    cellOccupied: (state, getters) => (cell) => {
        return !!getters.cellContent(cell);
    },
    ...
    withinBounds: (state, getters) => (position) => {
        return position.x >= 0 && position.x < getters.size &&
    ...
    tileMatchesAvailable(state, getters) {
        ...
        for (let x = 0; x < state.game.size; x++) {
            for (let y = 0; y < state.game.size; y++) {
                tile = getters.cellContent({x: x, y: y});

                if (tile) {
                    for (let direction = 0; direction < 4; direction++) {
                        let vector = getters.getVector(direction);
                        let cell = {x: x + vector.x, y: y + vector.y};
                        let other = getters.cellContent(cell);
